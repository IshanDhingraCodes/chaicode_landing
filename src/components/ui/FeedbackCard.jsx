...
const FeedbackCard = ({ content, name, title }) => {
  ...
  // Generate a unique gradient ID per card (based on name or random string)
  const gradientId = `gradient-${name.replace(/\s+/g, "-").toLowerCase()}-${Math.random()
    .toString(36)
    .substr(2, 5)}`;

  return (
    <div
      className={`flex justify-between flex-col p-6 rounded-[20px] max-w-[370px] my-5 ${
        theme === "light-theme"
